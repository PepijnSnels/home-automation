...
const connect = (event) => {
    ...
    if (deviceName) {
        ...
        const chatPage = document.querySelector('#device-page')
        chatPage.classList.remove('hide')

        const socket = new SockJS('/test')
...
const onConnected = () => {
    ...
    stompClient.send("/app/view.newDevice",
        {},
        JSON.stringify({sender: deviceName, type: 'CONNECT'})
    )
...
const sendMessage = (event) => {
    ...
    if (messageContent && stompClient) {
        ...
        stompClient.send("/app/view.send", {}, JSON.stringify(chatMessage))
        messageInput.value = ''
    }
    event.preventDefault();
}
